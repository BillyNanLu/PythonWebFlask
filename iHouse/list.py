...
def search_txt_info():
    ...
    if request.args.get("addr"):
        ...
        result = (
            House.query.filter(House.address == addr)
    ...
    if request.args.get("rooms"):
        ...
        result = (
            House.query.filter(House.rooms == rooms_info)
            .order_by(House.publish_time.desc())
            .all()
        )
        return render_template("search_list.html", house_list=result)
...
def return_new_list(page):
    ...
    result = House.query.order_by(House.publish_time.desc()).paginate(
        page=page, per_page=10
    )
    return render_template(
        "list.html", house_list=result.items, page_num=result.page, total_num=total_num
    )
...
def return_hot_list(page):
    ...
    # 计算总的页码数，向上取整
    total_num = math.ceil(house_num / 10)
    result = House.query.order_by(House.page_views.desc()).paginate(
        page=page, per_page=10
    )
    return render_template(
        "list.html", house_list=result.items, page_num=result.page, total_num=total_num
...
# @list_page.add_app_template_filter("dealover")
...
# @list_page.add_app_template_filter("dealdirection")
